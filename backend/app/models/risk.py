...
class RiskResult(Base):
    ...
    __table_args__ = (
        Index("ix_risk_results_type_computed", "risk_type", "computed_at"),
        Index("ix_risk_results_entity", "entity_type", "entity_id"),
    )
    ...
    payload: Mapped[dict] = mapped_column(
        JSON
    )  # pd/lgd/ead/el or var/es/volatility/... + model_version
...
class StressResult(Base):
    ...
    __table_args__ = (
        Index("ix_stress_results_computed", "computed_at"),
        Index("ix_stress_results_portfolio", "portfolio_id"),
    )

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    portfolio_id: Mapped[str | None] = mapped_column(String(20), nullable=True)
    scenario_name: Mapped[str] = mapped_column(String(100))
    shocks: Mapped[dict] = mapped_column(
        JSON
    )  # e.g. {"equity_shock": -0.20, "rate_shock_bps": 150, ...}
